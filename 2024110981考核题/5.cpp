...
map<string,Mat>ImageCache::cache;//静态成员变量应该在类外初始化
...
public:
    ...
    void process() override {
        ...
        resize(srcImg, srcImg, config.targetSize);
        delete processed;
    }
};
...
public:
    ...
    void detectCorners() {
        ...
        for(auto corner : corners) {  
            keypoints.push_back(KeyPoint(corner, 1.0f,1,0,1));
        }
    }
...
public:
    ImageProcessingPipeline(const string& imagePath) {
        ...
        if(originalImage.empty()) {
            throw std::runtime_error("Image load failed");//改为异常实例 
        }
    }
...
int main() {
    ...
    try {
        ...
        Mat img = imread("input.jpg");
         pipeline.addProcessor(new GeometricTransformer(origin_img, config));
        pipeline.addProcessor(new ImageEnhancer(origin_img, config));
        pipeline.addProcessor(new ImageFilter(origin_img, config));
        pipeline.addProcessor(new FeatureExtractor(origin_img, config));
